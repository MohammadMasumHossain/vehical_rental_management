...
const createBooking = async (payload, user) => {
    const vehicleRes = await db_1.pool.query(`SELECT * FROM vehicles WHERE id=$1`, [
        payload.vehicle_id,
    ]);
    ...
    await db_1.pool.query(`UPDATE vehicles SET availability_status='booked' WHERE id=$1`, [payload.vehicle_id]);
    return {
        ...result.rows[0],
        vehicle: {
...
const updateBooking = async (id, status, user) => {
    ...
    if (status === "returned" || status === "cancelled") {
        const vehicleRes = await db_1.pool.query(`UPDATE vehicles 
       SET availability_status='available' 
       WHERE id=$1 
